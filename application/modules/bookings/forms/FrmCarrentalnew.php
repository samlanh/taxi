...
// 	        'style'=>'color:red',
	    ...
	    $rows_plate=$db->getAllVehicleNo();
	    $opt_plate = array(0=>$this->tr->translate("Vehicle Ref.No"),-1=>$this->tr->translate("Add Vehicle"));
	    $vehicle = new Zend_Dojo_Form_Element_FilteringSelect("vehicle");
	    $vehicle->setAttribs(array('dojoType'=>'dijit.form.FilteringSelect','class'=>"fullside",'autoComplete'=>'false', 'queryExpr'=>'*${0}*',
	        'onchange'=>'getVehiclePopUp();'
	    ));
	    foreach ($rows_plate as $rs){
	        $opt_plate[$rs["id"]] = $rs["title"];
	    }
	    $vehicle->setMultiOptions($opt_plate);
