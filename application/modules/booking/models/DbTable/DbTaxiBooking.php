...
class Booking_Model_DbTable_DbTaxiBooking extends Zend_Db_Table_Abstract
{
    ...
    function createSessionTaxi($data,$step=1){
    	...
    	if($step==1){
    		...
    		$session->return_time = $data['pickup_time'].":".$data['pickup_minute'];
    		...
    		$session->trip_type = $data["trip_type"];
...
//     		$session-> productAvailable= $db_globle->getEquipment($array);
    	...
    	elseif($step==2){
    		...
    		$row = $this->getTaxiPriceByLocation($data,$pickup_location["id"],$return_location["id"]);
    		if($session->trip_type==1){
    		...
    		$session->time = $this->getTimeArrived($data,$pickup_location["id"],$return_location["id"]);
    	}elseif($step==3){
    ...
    public function addProductRental($data){
    	...
    	try{
    		...
    		$a = time($session->return_time);
    		$b = time($session->time);
    		$time = gmdate("h:i:s", $a+$b);
    		
    		$taxi_price = $session->taxi_price;
    		
    		if($data["payment_type"]==4){
    			$total_fee = $taxi_price;
    			//     			$deposit_fee = 0;
    			$total_pay = $data["cash_pay"];
    		}else{
    			$total_fee = $taxi_price;
    			//     			$deposit_fee = $diposit;
    			$total_pay = $taxi_price;
    		}
