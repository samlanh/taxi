...
class Booking_Model_DbTable_DbGuideBooking extends Zend_Db_Table_Abstract
{
    ...
    function createSessionGuide($data,$step=1){
...
//     	return $sql;
    ...
    public function addProductRental($data){
    	...
    	try{
	    	...
	    	foreach ($rows as $rs){
				if($rental_type==1){
					$total += (($rs["driver_price"]*$rs["amount_rent"])*$total_day);
				}else{
    		...
    		$total_payment = $total+$total_other_fee;
    	...
    	if($data["payment_type"]==1){
    		...
    		if(!empty($rows)){
    			foreach ($rows as $rs){
    				...
    				$arr_product = array(
    						'book_id'	=>	$book_id,
							'item_id'	=>	$rs["driver_id"],
							'item_name'	=>	$rs["driver_name"],
							'rent_num'	=>	$rs["amount_rent"],
							'price'		=>	$price,
							'total'		=>	$total_payment,
							'total_paymented'	=>	$total_payment,
							'status'	=>	1,
							'item_type'	=>	2
    				);
