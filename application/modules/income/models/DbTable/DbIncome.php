...
class Income_Model_DbTable_DbIncome extends Zend_Db_Table_Abstract
{
    ...
    function getAllIncome($search = null){
    	...
    	$sql = " SELECT  id,invoice,cheque_no,
			         (SELECT name_en FROM tb_view WHERE `key_code`=ln_income.payment_type AND `type`=15 LIMIT 1) AS payment_type,title,
			         CONCAT('$ ',total_amount),DATE_FORMAT(create_date, '%d-%m-%Y') AS create_date,
			        (SELECT first_name FROM rms_users WHERE rms_users.id=ln_income.user_id LIMIT 1) AS user_name,`status`
			        FROM ln_income ";
    	$where='';
	...
	public function addIncome($data){
		...
		try{
			...
			$income_id = $this->insert($arr_data);
			...
			foreach ($ids as $key => $i)
			{
				$data_item= array(
						'income_id' 		=> 	$income_id,
						'income_type_id' 	=> 	$data['expense_id_'.$i],
						'total_amount'     	=>		$data['price_'.$i],
						'description'      	=>		$data['note_'.$i],
						'status'      		=> 1,
				);
				$this->_name='ln_income_detail';
	...
	public function updateIncome($data){
		...
		try{
			...
			$sql =" DELETE FROM ln_income_detail WHERE income_id=".$data["id"];
			$db->query($sql);
			$ids=explode(',',$data['record_row']);
			foreach ($ids as $key => $i)
			{
	...
	function getIncomeType(){
		$db=$this->getAdapter();
		$sql=" SELECT id,title AS `name` FROM ln_income_type WHERE `status`=1 order by id desc";
		return $db->fetchAll($sql);
	}
	
	public function getAllIncomeType(){
		$tr = Application_Form_FrmLanguages::getCurrentlanguage();
		$rows = $this->getIncomeType();
		$options = '';
		$options .= '<option value="0">'.$tr->translate("SELECT_INCOME_TYPE").'</option>';
	...
	function getIncomeDetail($id){
		$db=$this->getAdapter();
		$sql=" SELECT id,income_type_id,description,total_amount FROM ln_income_detail WHERE income_id=$id";
		return $db->fetchAll($sql);
	}
