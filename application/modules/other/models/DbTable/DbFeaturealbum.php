...
class Other_Model_DbTable_DbFeaturealbum extends Zend_Db_Table_Abstract
{
	...
	function updateFeaturealbum($_data,$label_name){
		$db = $this->getAdapter();
		$db->beginTransaction();
		try{
			...
			$value='';
    		foreach ($ids as $i){
				if(empty($value)){$value= $_data['gallaryhomepage'.$i];}else{ $value= $value.",".$_data['gallaryhomepage'.$i];}
			}
				$_arr=array(
    				'value'      => $value,
