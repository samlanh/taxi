...
class agreement_Model_DbTable_DbProductAgreement extends Zend_Db_Table_Abstract
{
    ...
    function addProductAgreement($data){
    	$db = $this->getAdapter();
...
// 		return $db->fetchAll($sql);
	...
	function getProductBooking($booking_id){
		$db = $this->getAdapter();
		$sql="SELECT * FROM `ldc_booking_detail` WHERE book_id=$booking_id AND item_type=3 ";
		return $db->fetchAll($sql);
	}
